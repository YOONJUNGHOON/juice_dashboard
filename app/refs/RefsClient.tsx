  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/refs', {
        ...
        body: JSON.stringify({ label: newLabel, memo: newMemo, url: newUrl }),
      })
  ...
  async function handleSave(id: string) {
    ...
    try {
      const res = await fetch(`/api/refs/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ label: editLabel, memo: editMemo, url: editUrl }),
      })
  ...
  return (
    <>
        ...
        >
          <div className="flex gap-3">
            <div className="flex-1">
              <label className="block text-xs font-medium mb-1" style={{ color: 'var(--text-muted)' }}>사이트</label>
              <input
      ...
      >
        {/* Table header */}
        <div
          className="flex items-center px-5 py-2.5 text-xs font-semibold"
          style={{ borderBottom: '1px solid var(--border)', background: 'var(--bg)', color: 'var(--text-muted)' }}
        >
          {editMode && <div className="w-10 shrink-0" />}
          <span className="flex-1">사이트</span>
          <span className="flex-1 ml-4">메모</span>
          <span className="flex-[2] ml-4">Link</span>
          {editMode && <div className="w-20 shrink-0" />}
        </div>
        ...
        {initialLinks.length === 0 ? (
          ...
          initialLinks.map((link, i) => {
            ...
            // Edit mode: delete button on left + click row to edit
            if (editMode) {
              return (
                <div
                  key={link.id}
                ...
                >
                  ...
                  <span className="flex-1 ml-4 text-xs truncate" style={{ color: 'var(--text-muted)' }}>{link.memo ?? '—'}</span>
                  <span className="flex-[2] ml-4 text-xs font-mono truncate" style={{ color: 'var(--text-muted)' }}>{link.url}</span>
            ...
            return (
              <a
                ...
                className="ref-row flex items-center px-5 py-3.5 text-sm transition-colors"
                style={{ ...borderStyle, color: 'var(--text-primary)', textDecoration: 'none' }}
              >
                <span className="font-medium flex-1">{link.label}</span>
                <span className="flex-1 ml-4 text-xs truncate" style={{ color: 'var(--text-muted)' }}>{link.memo ?? '—'}</span>
                <span className="flex-[2] ml-4 text-xs font-mono truncate flex items-center gap-1" style={{ color: 'var(--text-muted)' }}>
                  {link.url}
